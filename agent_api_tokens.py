...
def _metrics_init(opp_id: str):
    ...
    METRICS[opp_id] = {
        ...
        "bq_bytes_written": 0,
        "bq_rows_written": 0,
        "by_agent": {},          # agent_name -> {input_tokens, output_tokens}
    }
...
def _metrics_add_tokens(opp_id: str, prompt: int, candidates: int, total: int,
    ...
    if not m:
        return
    pin, pout = int(prompt or 0), int(candidates or 0)
    m["input_tokens"] += pin
    m["output_tokens"] += pout
    m["total_tokens"] += int(total or pin + pout)
    # per-agent split, used for the daily BigQuery token table
    name = (agent or "unknown").strip() or "unknown"
    a = m["by_agent"].setdefault(name, {"input_tokens": 0, "output_tokens": 0})
    a["input_tokens"] += pin
    a["output_tokens"] += pout
...
REQUIRED_LABELS = {
    "R1_po_signed_contract": "Is a signed purchase order attached?",
    "R2_quote_to_customer": "Is the customer quote attached?",
    "R3_vendor_quote_clear_instruction": "Is the vendor quote attached and the vendor identified?",
    "R4_site_id": "Is the Site ID provided?",
    "R5_site_contact_info": "Are the site contact details provided?",
    "R6_summary_sold_designed_hld": "Is the solution summary or design attached?",
    "R7_summary_content_complete": "Is the summary complete?",
}

CHECK_LABELS = {
    "C1_leadtime_aligned_kpis": "Is the leadtime aligned to product KPIs?",
    "C2_circuit_order_form_attached": "Is the Circuit Order Form attached?",
    "C3_po_quote_attached_signed": "Is the PO or quote signed?",
    "C4_vendor_clear": "Is the vendor clearly identified?",
    "C5_third_party_vendor_docs": "Are third-party vendor quotes attached?",
    "C6_infra_orders_product_speed": "Are the infrastructure orders correct?",
    "C7_site_info_correct": "Is the site information correct?",
    "C8_site_contact_details": "Are the site contact details complete?",
    "C9_summary_matches_design_form": "Does the summary match the design and order form?",
    "C10_correct_customer_account": "Is the correct customer account used?",
    "C11_lan_ip_for_dia": "Are the customer LAN IP details provided?",
    "C12_public_ip_range_approved": "Has the public IP range been approved?",
    "C13_standard_offering_slash30": "Does the IP range match the standard offering?",
    "C14_kit_router_fulfilment": "Is the kit, router and fulfilment correct?",
    "C15_power_antenna_cables_licences": "Are all accessories included?",
    "C16_third_party_install_in_quote": "Is third-party installation included in the quote?",
    "C17_contract_term_matches": "Does the contract term match the order?",
    "C18_bend_info_correct": "Is the B-End information correct?",
    "C19_qos_af_ef_correct": "Is the QoS configuration correct?",
    "C20_cease_details_included": "Are cease details included?",
    "C21_upgrade_downgrade_stated": "Is the upgrade or downgrade stated?",
    "C22_referenced_files_attached": "Are all referenced files attached?",
    "C23_vendor_quote_still_valid": "Is the vendor quote still valid?",
    "C24_vf_quote_to_customer_attached": "Is the Vodafone quote to the customer attached?",
    "C25_leadtime_indicated_to_customer": "Has a leadtime been given to the customer?",
    "C26_vendor_quotations_attached": "Are the vendor quotations attached?",
    "C27_tech_spec_commercial_approval_margin": "Is commercial approval included?",
}

BASE_CHECK_LABELS = {
    "B1_customer_name_in_bsp": "Does the customer name exist in BSP?",
    "B2_delivery_address_in_bsp": "Does the delivery address exist in BSP?",
    "B3_billing_entity_matches": "Does the billing entity match BSP?",
    "B4_opp_ref_matches_vbop": "Does the opportunity reference match VBOP?",
    "B5_po_number_consistent": "Is the PO number consistent across documents?",
    "B6_cost_less_than_sell": "Is the sell price higher than the vendor cost?",
    "B7_product_codes_in_docs": "Are all product codes present in the vendor quote?",
    "B8_product_codes_in_bsp_mdm": "Do all product codes exist in BSP?",
    "B9_quantities_align": "Do the quantities align across documents?",
    "B10_vendor_quote_not_expired": "Is the vendor quote within its validity period?",
    "B11_contact_details_complete": "Are the order contact details complete?",
    "B13_billing_address_in_bsp": "Does the billing address exist in BSP?",
}

CIRCUIT_CHECK_LABELS = {
    "X1_contract_term_present": "Is the contract term provided?",
    "X2_premise_id_siro": "Is the Premise ID provided?",
    "X3_service_type_matches": "Does the service type match the documents?",
    "X4_speed_matches": "Does the circuit speed match the order form?",
    "X5_product_name_type_exist": "Is the product name or type provided?",
    "X7_po_matches_vbop": "Does the PO number match VBOP?",
    "X8_billing_freq_aligned": "Is the billing frequency provided?",
    "X9_po_type_recurring": "Is the PO type set to Recurring?",
}

# One-line description per check, shown under the title on the validation page.
CHECK_DESCRIPTIONS = {
    # Required information
    "R1_po_signed_contract": "A customer purchase order or signed contract must be present in the opportunity folder.",
    "R2_quote_to_customer": "The quote issued to the customer must be present.",
    "R3_vendor_quote_clear_instruction": "A quote from the vendor must be present, and the documents must state clearly which vendor to order from.",
    "R4_site_id": "An identifier for the installation site must be provided.",
    "R5_site_contact_info": "Contact details for the site must be provided.",
    "R6_summary_sold_designed_hld": "A summary of what was sold must be present, plus the HLD or design document where a solution design was involved.",
    "R7_summary_content_complete": "The summary must cover services supplied, deployment, engineering requirements, and leadtime.",
    # Document checks
    "C1_leadtime_aligned_kpis": "If a leadtime was quoted to the customer, it must match our published product KPIs.",
    "C2_circuit_order_form_attached": "Circuit orders must include the completed Circuit Order Form.",
    "C3_po_quote_attached_signed": "The attached purchase order or quote must carry a signature.",
    "C4_vendor_clear": "The documents must state which vendor the order is placed with.",
    "C5_third_party_vendor_docs": "Where a third party is used (Eir, EIL, SAB, Enet, Siro, Ripplecom/Host), their quote must be attached.",
    "C6_infra_orders_product_speed": "Infrastructure orders must be attached and match the product and speed being ordered.",
    "C7_site_info_correct": "Site ID, site name, and site address must all be present and correct.",
    "C8_site_contact_details": "Site contact name, email, and mobile number must all be provided.",
    "C9_summary_matches_design_form": "The order summary must be consistent with the attached design and order form.",
    "C10_correct_customer_account": "The order must be on the right customer account and company name so billing is correct.",
    "C11_lan_ip_for_dia": "Customer LAN IP details are required for DIA orders.",
    "C12_public_ip_range_approved": "Any customer request for public address ranges must carry approval.",
    "C13_standard_offering_slash30": "The standard offering for customer IP ranges is a /30.",
    "C14_kit_router_fulfilment": "The kit must be correct for the product, the router compatible with it, and fulfilment set correctly as run rate or ship back.",
    "C15_power_antenna_cables_licences": "Power supplies, antennas, extension cables, and licences must all be on the order.",
    "C16_third_party_install_in_quote": "Where a third party installs, that cost must appear in the quote.",
    "C17_contract_term_matches": "A contract term must be provided and must match the order.",
    "C18_bend_info_correct": "Where required, B-End details must be completed and correct.",
    "C19_qos_af_ef_correct": "Where required, Quality of Service must be completed, including AF and EF percentages on the order form.",
    "C20_cease_details_included": "If services are to be ceased once the new service completes, the cease details must be on the request.",
    "C21_upgrade_downgrade_stated": "Upgrades and downgrades must be clearly identified on the order.",
    "C22_referenced_files_attached": "Any email or file mentioned in the comments must be attached.",
    "C23_vendor_quote_still_valid": "The vendor quote must not be out of date.",
    "C24_vf_quote_to_customer_attached": "The Vodafone quote issued to the customer must be present.",
    "C25_leadtime_indicated_to_customer": "The leadtime indicated to the customer must be recorded.",
    "C26_vendor_quotations_attached": "Quotations from the vendor must be present.",
    "C27_tech_spec_commercial_approval_margin": "The tech spec must include commercial approval with margin, via pricing tool, EDRA, or commercial manager email.",
    # BSP checks
    "B1_customer_name_in_bsp": "The customer name on the tech spec must match a customer record in BSP.",
    "B2_delivery_address_in_bsp": "The delivery address must already exist in BSP.",
    "B3_billing_entity_matches": "The billing entity named in the documents must match the invoicing customer name held in BSP.",
    "B4_opp_ref_matches_vbop": "The opportunity reference in the tech spec must match the one stored in VBOP.",
    "B5_po_number_consistent": "The customer PO number must be consistent across VBOP, the customer PO, or an email confirmation.",
    "B6_cost_less_than_sell": "The price Vodafone sells at must be higher than the vendor cost price.",
    "B7_product_codes_in_docs": "Every product referenced in the tech spec must be identifiable in the vendor quote, by code or description.",
    "B8_product_codes_in_bsp_mdm": "Every product code on the order must exist in BSP or MDM.",
    "B9_quantities_align": "Quantities must be consistent across the tech spec, quotes, and customer PO. The vendor quote may be higher.",
    "B10_vendor_quote_not_expired": "The vendor quote must be within its 30-day validity period and not expired.",
    "B11_contact_details_complete": "The order contact name and details in the tech spec must be complete.",
    "B13_billing_address_in_bsp": "The billing address must be present in the documents and already exist in BSP.",
    # Circuit checks
    "X1_contract_term_present": "Circuit orders must state the contract term, in the circuit order form or tech spec.",
    "X2_premise_id_siro": "A valid Premise ID is required for orders placed with SIRO.",
    "X3_service_type_matches": "The service type must be stated in the tech spec and match the supporting documents.",
    "X4_speed_matches": "The circuit speed in the tech spec must match the speed on the customer order form.",
    "X5_product_name_type_exist": "Either the product name or the product type must be provided.",
    "X7_po_matches_vbop": "The PO number in the circuit order form must match the one in VBOP.",
    "X8_billing_freq_aligned": "The billing frequency must be stated in the tech spec.",
    "X9_po_type_recurring": "For recurring circuit orders the PO type must be set to Recurring.",
}
    ...
    def items(section, labels):
        ...
        for key, label in labels.items():
            ...
            out.append({
                "key": key,
                "label": label,
                "description": CHECK_DESCRIPTIONS.get(key, ""),
                "ok": res == "Y",
...
_MOCK_BASE_FAIL = {"B1_customer_name_in_bsp": "raise_mdg", "B8_product_codes_in_bsp_mdm": "raise_mdm",
                   "B13_billing_address_in_bsp": "raise_mdg"}
_MOCK_CIRCUIT_FAIL = {"X2_premise_id_siro": "manual_correction"}
            ...
            try:
                ...
                if um is not None:
                    _metrics_add_tokens(
                        ...
                        agent=author or _current.get("node") or "unknown",
                    )
...
async def process(opp_id: str, request: Request = None):
    ...
    try:
        ...
        pass
    # Append this run's token usage to the daily per-agent BigQuery table.
    if not AGENT_MOCK:
        try:
            from vf_quote_to_order_agent_configured.Agent.tools import (  # type: ignore
                record_daily_tokens,
            )
            _m = METRICS.get(opp_id) or {}
            record_daily_tokens(_m.get("by_agent") or {})
        except Exception as _tok_exc:  # noqa: BLE001
            print(f"[tokens] could not record daily tokens: {_tok_exc}")
...
class FeedbackItem(BaseModel):
    ...
    option: str            # raise_mdg | raise_mdm | raise_sales | manual_correction | no_action
    comment: str = ""
    by: str = "ui"
