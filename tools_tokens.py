...
# ─────────────────────────────────────────────────────────────────────
# Daily token consumption table (BigQuery)
# Two rows per day, one per app:
#   q2o  "Quote to Order"           = retrieval + BSP extraction + email drafter
#   pcn  "Product Code Navigator"   = product code navigator (separate app)
# Rows accumulate: running the app again on the same day increments the
# existing row rather than adding a new one.
#   columns: date DATE, app_acronym STRING, name STRING,
#            input_tokens INT64, output_tokens INT64, total_tokens INT64
# ─────────────────────────────────────────────────────────────────────
BQ_TOKENS_TABLE = os.getenv(
    "BQ_TOKENS_TABLE",
    "vf-ie-datahub.vfie_dh_lake_customer_complex_fixed_s."
    "complex_fixed_daily_tokens_consumption",
)

# Which app each ADK agent's tokens are attributed to.
AGENT_TO_APP = {
    "qto_retrieval_agent":  ("q2o", "Quote to Order"),
    "bsp_extraction_agent": ("q2o", "Quote to Order"),
    "email_agent":          ("q2o", "Quote to Order"),
    "product_code_agent":   ("pcn", "Product Code Navigator"),
}
DEFAULT_APP = ("q2o", "Quote to Order")


def _tokens_bq_client():
    """BigQuery client for the token table (billed to BQ_PROJECT)."""
    from google.cloud import bigquery
    return bigquery.Client(project=os.getenv("BQ_PROJECT", PROJECT_ID))


def ensure_token_table():
    """Create the daily token consumption table if it does not exist."""
    from google.cloud import bigquery
    client = _tokens_bq_client()
    schema = [
        bigquery.SchemaField("date", "DATE", mode="REQUIRED"),
        bigquery.SchemaField("app_acronym", "STRING", mode="REQUIRED"),
    ...
    table = bigquery.Table(BQ_TOKENS_TABLE, schema=schema)
    table.time_partitioning = bigquery.TimePartitioning(field="date")
    try:
        client.create_table(table, exists_ok=True)
        return {"status": "success", "table": BQ_TOKENS_TABLE}
    except Exception as exc:  # noqa: BLE001
        print(f"[tokens] could not create table {BQ_TOKENS_TABLE}: "
              f"{type(exc).__name__}: {exc}")
        return {"status": "error", "message": str(exc)}


def group_tokens_by_app(by_agent: dict) -> dict:
    """Roll per-agent token counts up to the two apps (q2o and pcn).

    by_agent: {agent_name: {"input_tokens": n, "output_tokens": n}}
    returns:  {acronym: {"name": str, "input_tokens": n, "output_tokens": n}}
    """
    apps = {}
    for agent, t in (by_agent or {}).items():
        acronym, name = AGENT_TO_APP.get(agent, DEFAULT_APP)
        a = apps.setdefault(acronym, {"name": name, "input_tokens": 0,
                                      "output_tokens": 0})
        a["input_tokens"] += int(t.get("input_tokens", 0) or 0)
        a["output_tokens"] += int(t.get("output_tokens", 0) or 0)
    return apps


def record_daily_tokens(by_agent: dict, usage_date=None) -> dict:
    """Add this run's tokens to the daily per-app totals.

    Groups the agents into q2o and pcn, then MERGEs so each (date, app)
    keeps one row whose totals increment on every run that day.
    """
    import datetime as _dt
    apps = group_tokens_by_app(by_agent)
    if not apps:
        return {"status": "skipped", "message": "no token data"}

    day = usage_date or _dt.date.today().isoformat()
    try:
        from google.cloud import bigquery
        client = _tokens_bq_client()
        ensure_token_table()
        ...
        written = 0
        for acronym, t in apps.items():
            tin, tout = int(t["input_tokens"]), int(t["output_tokens"])
            if tin == 0 and tout == 0:
                continue
            sql = f"""
                MERGE `{BQ_TOKENS_TABLE}` T
                USING (SELECT DATE(@day) AS date, @app AS app_acronym,
                              @name AS name, @tin AS input_tokens,
                              @tout AS output_tokens) S
                ON T.date = S.date AND T.app_acronym = S.app_acronym
                WHEN MATCHED THEN UPDATE SET
                    input_tokens  = T.input_tokens  + S.input_tokens,
                    output_tokens = T.output_tokens + S.output_tokens,
                    total_tokens  = T.total_tokens  + S.input_tokens + S.output_tokens
                WHEN NOT MATCHED THEN INSERT
                    (date, app_acronym, name, input_tokens, output_tokens, total_tokens)
                    VALUES (S.date, S.app_acronym, S.name, S.input_tokens,
                            S.output_tokens, S.input_tokens + S.output_tokens)
            """
            job_config = bigquery.QueryJobConfig(query_parameters=[
                bigquery.ScalarQueryParameter("day", "STRING", day),
                bigquery.ScalarQueryParameter("app", "STRING", acronym),
                bigquery.ScalarQueryParameter("name", "STRING", t["name"]),
                bigquery.ScalarQueryParameter("tin", "INT64", tin),
                bigquery.ScalarQueryParameter("tout", "INT64", tout),
            ])
            client.query(sql, job_config=job_config).result()
            written += 1
            print(f"[tokens] {day} {acronym} ({t['name']}): "
                  f"+{tin:,} in / +{tout:,} out")
        return {"status": "success", "apps_updated": written,
                "table": BQ_TOKENS_TABLE, "date": day}
    except Exception as exc:  # noqa: BLE001
        print(f"[tokens] daily token write failed: {type(exc).__name__}: {exc}")
        return {"status": "error", "message": f"{type(exc).__name__}: {exc}"}
